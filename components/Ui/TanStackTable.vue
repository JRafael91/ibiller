		...
		<div :class="styles({ class: props.class })">
			<UiTable :class="tableClass">
										...
										<UiDropdownMenu>
											...
											<UiDropdownMenuContent class="w-48">
												<UiDropdownMenuRadioGroup
													:model-value="(header.column.getFilterValue() as string) ?? 'Todos'"
													@update:model-value="
														(e) => header.column.setFilterValue(e == 'Todos' ? undefined : e)
													"
												>
													<UiDropdownMenuRadioItem
														v-for="item in ['Todos', 'Success', 'Processing', 'Failed']"
														:key="item"
														:value="item"
														:title="item"
														:text-value="item"
				...
				<UiTableBody>
					...
					>
						<slot :table="table" name="empty"> Sin datos disponibles. </slot>
					</UiTableEmpty>
				</UiTableBody>
			</UiTable>
		</div>
